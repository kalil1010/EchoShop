...
Email service is handled by Supabase's built-in SMTP configuration. Configure your SMTP provider in:
- **Supabase Dashboard** → Authentication → Settings → SMTP Settings

No additional environment variables are required for email functionality when using Supabase's built-in SMTP.
...
# Email Service
# Configure SMTP in Supabase Dashboard → Authentication → Settings → SMTP Settings
# No environment variables needed - Supabase handles email sending
...
- Email service is configured in Supabase Dashboard → Authentication → Settings → SMTP Settings.
