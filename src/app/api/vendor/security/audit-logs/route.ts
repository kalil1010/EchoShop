...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch audit logs for vendor actions
    const { data: events, error } = await supabase
      .from('event_log')
      .select('*')
      .eq('user_id', userId)
      .or('event_name.like.vendor_%,event_name.like.security_%,event_name.like.auth_%')
    ...
    const logs = (events || []).map((event) => {
      ...
      const action = event.event_name.replace(/^vendor_|^security_|^auth_/, '').replace(/_/g, ' ')
