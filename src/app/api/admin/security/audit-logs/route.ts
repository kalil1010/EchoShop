...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch audit logs for admin/owner actions
    const { data: events, error } = await supabase
      .from('event_log')
      .select('*')
      .eq('user_id', userId)
      .or('event_name.like.admin_%,event_name.like.security_%,event_name.like.auth_%')
    ...
    const logs = (events || []).map((event) => {
      ...
      const action = event.event_name.replace(/^admin_|^security_|^auth_/, '').replace(/_/g, ' ')
